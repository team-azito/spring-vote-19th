...
class UserApiTest extends ApiTest {
    ...
    void testSignUp_Fail_WhenDuplicateUsername() {
        ...
        assertSoftly(
                softly -> {
                    ...
                    softly.assertThat(회원_가입_응답.body().jsonPath().getString("message")).isEqualTo(ExceptionCode.ALREADY_EXIST_USERNAME_EXCEPTION.getMessage());
                }
        );
    }
    ...
    void testSignUp_Fail_WhenDuplicateEmail() {
        ...
        assertSoftly(
                softly -> {
                    softly.assertThat(회원_가입_응답.statusCode()).isEqualTo(HttpStatus.BAD_REQUEST.value());
                    softly.assertThat(회원_가입_응답.body().jsonPath().getString("message")).isEqualTo(ExceptionCode.ALREADY_EXIST_EMAIL_EXCEPTION.getMessage());
                }
        );
    }
}
